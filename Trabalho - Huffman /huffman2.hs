...
put texto = do
    put' (numChar (freqSimb texto)) (quantChar (codificar texto (codHuffman (construirArvore (sort (freqSimb texto))))))
    put'' (freqSimb texto)
    put''' (codificar texto (codHuffman (construirArvore (sort (freqSimb texto)))))
...
main = do
    ...
    texto <- readFile arq
    --Aqui montamos a estrutura que vai armazenar todos os dados em binario
    let bs = P.runPut (put texto)
...
main' = do
    ...
    writeFile "textoDecodificado.txt" (decodificar (toBin (fromIntegral v) (toIntFromChar (toString binarios))) (construirArvore (sort $ toHuff freq)))
    return ()